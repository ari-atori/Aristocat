...
class PollBuilder(nextcord.ui.Modal):
	def __init__(self, number_of_boxes, is_default):
		...
		question_input = TextInput(label="Question", placeholder="Type your question here", max_length=500, required=True)
		self.add_item(question_input)

		if not self.is_default:
			for i in range(self.number_of_boxes):
				text_input = TextInput(label=f"Option {i + 1}", placeholder=f"Option {i + 1}", max_length=100, required=False)
...
class Poll(commands.Cog, name="poll"):
	...
	async def poll(self, interaction: Interaction):
		...
		embed.add_field(name="Default Poll", value="You will have three options in your poll that you don't get to specify. (👍, 👎, 🤷)", inline=False)
		...
		async def custom_poll_button_callback(interaction):
			modal = PollBuilder(int(string_select.values[0]), False)
			await interaction.response.send_modal(modal)
		
		async def default_poll_button_callback(interaction):
			modal = PollBuilder(3, True)
			await interaction.response.send_modal(modal)
